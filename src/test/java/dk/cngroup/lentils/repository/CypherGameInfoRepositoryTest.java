...
public class CypherGameInfoRepositoryTest {
    ...
    public void testTeamWithHintTaken() {
        ...
        List<CypherGameInfo> withHint = result.stream().filter((a) -> a.getCount() > 1).collect(Collectors.toList());

        assertEquals(1, withHint.size());
        assertEquals(team1.getTeamId(), withHint.get(0).getTeamId());
        assertEquals(cypher1.getCypherId(), withHint.get(0).getCypherId());
    }
